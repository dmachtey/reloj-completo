...
void display_init(void) {
    ...
    panel_minutes = CrearPanel(30, 60, 2, DIGITO_ALTO, DIGITO_ANCHO, DIGITO_ENCENDIDO, DIGITO_APAGADO, DIGITO_FONDO);
    panel_seconds = CrearPanel(170, 60, 2, DIGITO_ALTO, DIGITO_ANCHO, DIGITO_ENCENDIDO, DIGITO_APAGADO, DIGITO_FONDO);
    panel_tenths = CrearPanel(310, 60, 2, DIGITO_ALTO, DIGITO_ANCHO, DIGITO_ENCENDIDO, DIGITO_APAGADO, DIGITO_FONDO);
...
void display_task(void * pvParameters) {
    ...
    for (;;) {
        ...
        /* --- MODO CLOCK / CLOCK_SET --- */
                 ...
                 ((xQueueReceive(xClockQueue, &c, pdMS_TO_TICKS(2)) == pdTRUE) || mode_changed ||
...

        {
            ...
            if ((blink2) && (current_mode == MODE_CLOCK_SET) && (clk_set_sequence == CLK_SEQ_HR)) {
                Panel_SetOnColor(panel_minutes, DIGITO_APAGADO);
            }

            Panel_SetOnColor(panel_seconds, DIGITO_ENCENDIDO);
            if ((blink2) && (current_mode == MODE_CLOCK_SET) && (clk_set_sequence == CLK_SEQ_MIN)) {
                Panel_SetOnColor(panel_seconds, DIGITO_APAGADO);
            }

            if ((old_mins != mins) ||
                (blink2_changed && (current_mode == MODE_CLOCK_SET) && (clk_set_sequence == CLK_SEQ_HR)) ||
            ...
            if ((old_secs != secs) ||
                (blink2_changed && (current_mode == MODE_CLOCK_SET) && (clk_set_sequence == CLK_SEQ_MIN)) ||
                (clk_set_sequence != old_clk_seq)) {
                DibujarDigito(panel_seconds, 0, secs / 10);
                DibujarDigito(panel_seconds, 1, secs % 10);
                old_secs = secs;
        ...
        /* --- MODO ALARM / ALARM_SET --- */
                 ...
                 ((xQueueReceive(xAlarmQueue, &a, pdMS_TO_TICKS(2)) == pdTRUE) || mode_changed ||
                  ((current_mode == MODE_ALARM_SET) && blink))) {
            if (a.mode != MODE_ALARM && a.mode != MODE_ALARM_SET)
                continue;
            uint32_t mins = a.al_h; // horas de alarma
            uint32_t secs = a.al_m; // minutos de alarm

            Panel_SetOnColor(panel_minutes, DIGITO_ENCENDIDO);
            if ((blink2) && (current_mode == MODE_ALARM_SET) && (alarm_set_sequence == ALARM_SEQ_HR))
                Panel_SetOnColor(panel_minutes, DIGITO_APAGADO);
            ...
            Panel_SetOnColor(panel_seconds, DIGITO_ENCENDIDO);
            if ((blink2) && (current_mode == MODE_ALARM_SET) && (alarm_set_sequence == ALARM_SEQ_MIN))
                Panel_SetOnColor(panel_seconds, DIGITO_APAGADO);
