...
{
    switch (seq) {
        case ALARM_SEQ_IDLE: return "ALRM_SEQ_IDLE";
        case ALARM_SEQ_MIN:  return "ALRM_SEQ_MIN";
        case ALARM_SEQ_HR:   return "ALRM_SEQ_HR";
        case ALARM_SEQ_EN:   return "ALRM_SEQ_EN";
        default:             return "UNKNOWN_ALRM_SEQ";
    }
}
...
void mode_manager_init(void)
{
    ...
    ESP_LOGI(TAG, "Mode manager initialized, starting in %s",
    ...
    for (;;) {
        ...
        if (bits & EV_BIT_FUNC_CHANGE) {
            ...
            switch (current_mode) {
                ...
                case MODE_ALARM_RING: /* permanece hasta snooze/stop */ break;
        ...
        if (bits & EV_BIT_RESET) {
            if (current_mode == MODE_CLOCK_SET) {
                ...
                ESP_LOGI(TAG, "ALRM_SET sequencer -> %s",
                         alarm_seq_to_string(alarm_set_sequence));
            }
        }
